    ...
    private JsonObject saveVoucher(final PurchaseControllerHeaderModel header, final ArrayList<PurcahseControllerDetailModel> detail) {
        ...
        if (dataConnection != null) {
            try {
                ...
                String ac_name = lb.getData(dataConnection, "FNAME", "ACNTMST", "AC_CD", header.getAC_CD(), 0);
                for (int i = 0; i < detail.size(); i++) {
                    if (lb.getData(dataConnection, "ref_no", "TAG", "ref_no", detail.get(i).getPUR_TAG_NO(), 0).equalsIgnoreCase("")) {
                        if (detail.get(i).getTAG_NO().equalsIgnoreCase("")) {
                            if (header.getV_TYPE() == 1) {
                                String pref = "ANB" + getMonth() + (Calendar.getInstance().get(Calendar.YEAR) + "").substring(2);
                                String tag = lb.generateKey(dataConnection, "tag", "tag_no", pref, 11);
                        ...
                        sql = "insert into TAG (TAG_NO,SR_CD,IMEI_NO,SERAIL_NO,PUR_RATE,BASIC_PUR_RATE,pur_ref_no,DISC_RATE,NLC,MRP,BRANCH_CD,ref_no,IS_MAIN,PUR_DATE,ac_name) "
                                + "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                        ...
                        pstUpdate.setString(14, header.getV_DATE());
                        pstUpdate.setString(15, ac_name);
                        ...
                        sql = "update TAG set sr_cd=?,imei_no=?,SERAIL_NO=?,pur_rate=?,BASIC_PUR_RATE=?,DISC_RATE=?,NLC=?,MRP=?,PUR_DATE=?,ac_name=? where ref_no=?";
                        PreparedStatement pstUpdate = dataConnection.prepareStatement(sql);
                        ...
                        pstUpdate.setString(9, header.getV_DATE());
                        pstUpdate.setString(10, ac_name);
