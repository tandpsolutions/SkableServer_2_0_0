                ...
                if (detail.get(0).getV_type().equalsIgnoreCase("1")) {
                    for (int i = 0; i < detail.size(); i++) {
                        if (detail.get(i).getPur_tag_no().equalsIgnoreCase("")) {
                            ...
                            sql = "insert into TAG (TAG_NO,SR_CD,IMEI_NO,SERAIL_NO,PUR_RATE,BASIC_PUR_RATE,pur_ref_no,DISC_RATE,MRP,BRANCH_CD,ref_no,IS_MAIN) "
                                    + "values (?,?,?,?,?,?,?,?,?,?,?,1)";
                            ...
                            sql = "update TAG set sr_cd=?,imei_no=?,SERAIL_NO=?,pur_rate=?,BASIC_PUR_RATE=?,DISC_RATE=?,MRP=? where ref_no=?";
                            PreparedStatement pstUpdate = dataConnection.prepareStatement(sql);
                            ...
                            pstUpdate.setString(8, detail.get(i).getPur_tag_no());
                            pstUpdate.executeUpdate();
                            if (pstLocal != null) {
                                lb.closeStatement(pstLocal);
                            }
