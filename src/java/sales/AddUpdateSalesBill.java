            ...
            try {
                ...
                if (header.getRef_no().equalsIgnoreCase("")) {
                    ...
                    sql = "INSERT INTO VILSHD (INV_NO,V_DATE,V_TYPE,PMT_MODE,AC_CD,DET_TOT,BRANCH_CD,"
                            ...
                            + "BANK_CHARGES,REMARK,PMT_DAYS,DUE_DATE,head_disc,REF_CD,REF_NO) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                } else {
                    ...
                    sql = "UPDATE VILSHD set INV_NO=?,V_DATE=?,V_TYPE=?,PMT_MODE=?,AC_CD=?"
                            ...
                            + "ins_cd=?,ins_amt=?,bank_charges=?,remark=?,pmt_days=? ,DUE_DATE=?,head_disc=?,ref_cd=?  where ref_no=?";
                }
                PreparedStatement pstLocal = dataConnection.prepareStatement(sql);
                ...
                pstLocal.setDouble(24, header.getHead_disc());
                pstLocal.setString(25, header.getRef_cd());
                pstLocal.setString(26, header.getRef_no());
                pstLocal.executeUpdate();
                ...
                BranchMasterModel branch = lb.branchSetUp(header.getBRANCH_CD() + "");

                sql = "INSERT INTO PAYMENT (CASH_AMT, BANK_CD, BANK_NAME, BANK_BRANCH, CHEQUE_NO, CHEQUE_DATE, BANK_AMT, CARD_NAME, CARD_AMT, "
                        + "REF_NO,USER_ID,vou_date,bajaj_name,bajaj_amt,SFID,CASH_AC_CD,DISC_AMT,card_no,tid_no,bajaj_per ,bajaj_chg ,card_per, card_chg)"
                        + " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                pstLocal = dataConnection.prepareStatement(sql);
                ...
                pstLocal.setString(16, branch.getCash_ac_cd());
                pstLocal.setDouble(17, header.getDISC_AMT());
                pstLocal.setString(18, header.getCard_no());
                pstLocal.setString(19, header.getTid_no());
                pstLocal.setDouble(20, header.getBAJAJ_PER());
                pstLocal.setDouble(21, header.getBAJAJ_CHG());
                pstLocal.setDouble(22, header.getCARD_PER());
                pstLocal.setDouble(23, header.getCARD_CHG());
                pstLocal.executeUpdate();
