...
public class GetIntransitStockReport extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (dataConnection != null) {
            try {
                String sql = "SELECT s.ref_no,inv_no,v_date,from_loc,to_loc,approve_by,COUNT(s1.pur_tag_no) as qty FROM stktrfhd s LEFT JOIN "
                        + " stktrfdt s1 ON s.ref_no=s1.ref_no  WHERE v_date>=? AND v_date<=? and approve_by IS NULL"
                        + " GROUP BY s.ref_no,inv_no,v_date,from_loc,to_loc,approve_by";
