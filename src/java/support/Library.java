...
public class Library {
    ...
    public double getBalanceStockByDate(Connection dataConnection, String ac_cd, String date, int mode) throws SQLException {
        String sql = "select (sum(case when TRNS_ID='R' then PCS else 0 end) -sum(case when TRNS_ID='I' then pcs else 0 end)) as opb,"
                + "sum(case when TRNS_ID='R' then PCS*RATE else 0 end) "
    ...
    public JsonArray getSeriesMaster(Connection dataConnection, String field, String value) throws SQLException {
        String sql = "select SR_CD,SR_NAME,t.tax_cd,t.tax_name,SR_ALIAS from SERIESMST s left join modelmst m on "
                + "s.model_cd=m.model_cd "
...
//printToLogFile("Error at isNumber in Library", ex);
    ...
    public JsonArray getTagNoDetailSales(Connection dataConnection, String tagListForRandom, boolean flag, String loc) throws SQLException {
        String sql = "Select TAG_NO,s.sr_alias as SR_NAME,SR_NAME as ITEM_NAME,t.ref_no,s.sr_cd,t1.tax_cd"
                + ",t1.tax_name,t.IMEI_NO,t.SERAIL_NO,t.is_main,t.PUR_RATE,s.MOP  "
                + " from tag t LEFT JOIN SERIESMST s on t.SR_CD=s.SR_CD left join modelmst m on "
                + " s.model_cd=m.model_cd left join taxmst t1 on m.tax_cd=t1.tax_cd "
                + " WHERE t.tag_no in (" + tagListForRandom + ") and t.branch_cd =" + loc;
        if (flag) {
            sql += " and t.is_del=0";
        } else {
            sql += " and t.is_del=1";
        ...
        while (rsLocal.next()) {
            ...
            object.addProperty("MOP", rsLocal.getString("MOP"));
            array.add(object);
        }
        return array;
    }
    ...
    public JsonArray getSalesBill(Connection dataConnection, String ref_no) throws SQLException {
        String sql = "SELECT l.remark,l.REF_NO,l.INV_NO,l.V_DATE,l.V_TYPE,l.PMT_MODE,l.AC_CD,a.FNAME,a1.ADD1,p.MOBILE1,l.DET_TOT, a.TIN,l1.TAX_CD,l1.TAX_AMT,"
                ...
                + " pmt.DISC_AMT,'' as BUY_BACK_CD,BUY_BACK_MODEL,l.BUY_BACK_AMT,l.BUY_BACK_IMEI_NO,l.PART_NO,s2.SR_CD as INS_CD,"
                + " s2.SR_NAME AS INS_MODEL,l.INS_AMT,l.PMT_DAYS,l.BANK_CHARGES, l.ADVANCE_AMT,l.head_disc,a.ref_by,r.ref_name,pmt.card_no,pmt.tid_no,"
                + " pmt.card_chg,pmt.card_per,pmt.bajaj_per,pmt.bajaj_chg "
                ...
                + " LEFT JOIN ACNTMST card ON pmt.CARD_NAME=card.AC_CD LEFT JOIN seriesmst s2 ON l.INS_CD=s2.SR_CD left join refmst r on r.ref_cd=l.ref_cd "
                + " LEFT JOIN ACNTMST bajaj ON pmt.BAJAJ_NAME=bajaj.AC_CD WHERE l.REF_NO='" + ref_no + "' order by tag_no,is_main desc";
        PreparedStatement pstLocal = dataConnection.prepareStatement(sql);
        ResultSet rsLocal = pstLocal.executeQuery();
        JsonArray array = new JsonArray();
        while (rsLocal.next()) {
            ...
            object.addProperty("AMT", rsLocal.getString("AMT"));
            object.addProperty("BASIC_AMT", rsLocal.getString("BASIC_AMT"));
            object.addProperty("HEAD_DISC", rsLocal.getString("HEAD_DISC"));
            object.addProperty("PUR_TAG_NO", rsLocal.getString("PUR_TAG_NO"));
            object.addProperty("TAG_DEL", rsLocal.getString("TAG_DEL"));
            object.addProperty("REMARK", rsLocal.getString("REMARK"));
            object.addProperty("DISC_RATE", rsLocal.getString("DISC_RATE"));
            ...
            object.addProperty("DISC_AMT", rsLocal.getString("DISC_AMT"));
    ...
    public JsonArray getModelMaster(Connection dataConnection, String field, String value) throws SQLException {
        String sql = "select MODEL_CD,MODEL_NAME,BRAND_NAME from MODELMST m left join BRANDMST b on m.brand_cd=b.brand_cd WHERE " + field + " like '%" + value + "%'";
        PreparedStatement pstLocal = dataConnection.prepareStatement(sql);
        ResultSet rsLocal = pstLocal.executeQuery();
        JsonArray array = new JsonArray();
        while (rsLocal.next()) {
    ...
    public JsonArray getCashDetail(Connection dataConnection, String field, String value) throws SQLException {
        String sql = "SELECT c1.SR_NO,c.BRANCH_CD,c.REF_NO,VDATE,a.FNAME,c1.BAL,c1.REMARK,c.USER_ID,c.EDIT_NO,c.TIME_STAMP,a.AC_CD,o.DOC_REF_NO,o.INV_NO,o.DOC_CD FROM CPRHD "
        ...
        while (rsLocal.next()) {
            ...
            final String doc_ref_no = getData(dataConnection, "DOC_REF_NO", "billadjst", "DR_SR_NO=" + rsLocal.getString("SR_NO") + " and DR_DOC_REF_NO", rsLocal.getString("REF_NO"), 0);
            object.addProperty("IS_EDITED", (doc_ref_no.equalsIgnoreCase("") ? "1" : "0"));
            array.add(object);
        }
        return array;
    }
    ...
    public JsonArray getCashDetailRecpt(Connection dataConnection, String field, String value) throws SQLException {
        ...
        while (rsLocal.next()) {
            ...
            object.addProperty("COMPANY_TIN", companySetUp().getTin_no());
            object.addProperty("COMPANY_CST", companySetUp().getCst_No());
            final String doc_ref_no = getData(dataConnection, "DOC_REF_NO", "billadjst", "CR_SR_NO=" + rsLocal.getString("SR_NO") + " and CR_DOC_REF_NO", rsLocal.getString("REF_NO"), 0);
            object.addProperty("IS_EDITED", (doc_ref_no.equalsIgnoreCase("") ? "1" : "0"));
            array.add(object);
        }
        return array;
    ...
    public JsonArray getJournalDetail(Connection dataConnection, String field, String value) throws SQLException {
        String sql = "select c.REF_NO,VDATE,a.FNAME,c1.AMT,c1.PART,c.USER_ID,c.EDIT_NO,c.TIME_STAMP,a.AC_CD,c1.DRCR from JVHD "
    ...
    public JsonArray getBankDetail(Connection dataConnection, String field, String value) throws SQLException {
        String sql = "SELECT c1.sr_no,c.BRANCH_CD,c.REF_NO,VDATE,a.FNAME,c1.BAL,c1.REMARK,c.USER_ID,c.EDIT_NO,c.TIME_STAMP,a.AC_CD,o.DOC_REF_NO,o.INV_NO,o.DOC_CD,"
                + "a1.AC_CD as bank_cd,"
    ...
    public BranchMasterModel branchSetUp(String branch_cd) {
        boolean flag = false;
        BranchMasterModel clSysEnv = new BranchMasterModel();
        Connection con = null;
        try {
            ...
            ResultSet rsLocal = pstLocal.executeQuery();
            if (rsLocal.next()) {
                clSysEnv.setBranch_cd(branch_cd);
                clSysEnv.setCash_ac_cd(rsLocal.getString("cash_ac_cd"));
            }
            closeResultSet(rsLocal);
            closeStatement(pstLocal);
    ...
    public String getData(Connection dataConnection, String sql) {
        String data = "";
        try {
            PreparedStatement pstLocal = dataConnection.prepareStatement(sql);
            ResultSet rsLocal = pstLocal.executeQuery();
    ...
    public void displaySalesVoucherEmail(Connection dataConnection, SalesControllerHeaderModel header, ArrayList<SalesControllerDetailModel> detail) {
        ...
        try {
            ...
            content += "<html><body>"
                    ...
                    + "<label></label><label>" + header.getAc_name() + "</label><br/>"
                    + "        <br/>"
                    + "        <table border=\"1\" width=\"100%\" align=\"center\">";
            content += " <tr>"
                    + "<td>" + 1 + "</td>"
                    + "<td>"
                    + "    " + detail.get(0).getSR_NAME() + ""
                    + "</td>"
                    + "<td>"
            ...
            content += "</table> <br/><br/>"
                    ...
                    + "</tr>"
                    + "        </table></body></html>";
            SendMailSSL1 se = new SendMailSSL1("atik22capplenberry@gmail.com");
            se.sendEmailContent(content);
            se = new SendMailSSL1("atul22c@gmail.com");
            se.sendEmailContent(content);
            se = new SendMailSSL1("jet_viral@yahoo.co.in");
            se.sendEmailContent(content);
            se = new SendMailSSL1("manzilpatel007@gmail.com");
            se.sendEmailContent(content);
            se = new SendMailSSL1("brijesh.patel.bill@gmail.com");
            se.sendEmailContent(content);
        } catch (Exception ex) {
